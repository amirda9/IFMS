...
const [errortext,setErrortext]=useState("")
  ...
  const [helixfactor,setHelixfactor]=useState(SettingsGet?.data?.optical_route?.helix_factor || 1.02)
  const [ior,setIor]=useState<{1310:number,1490:number,1550:number,1625:number}>(SettingsGet?.data?.optical_route?.wavelengths?.IOR || {1310:1.4678,1490:1.4679,1550:1.468,1625:1.4681});
  const [rbs,setRbs]=useState<{1310:number,1490:number,1550:number,1625:number}>(SettingsGet?.data?.optical_route?.wavelengths?.RBS || {1310:-79.01,1490:-79.02,1550:-79.03,1625:-79.04});
  const [fiberType,setFiberType]=useState(SettingsGet?.data?.optical_route?.fiber_type || "G.656")

  const onSaveButtonClick=()=>{
    if(!helixfactor || !ior[1310] || !ior[1490] || !ior[1550] || !ior[1625] || !rbs[1310] || !rbs[1490] || !rbs[1550] || !rbs[1625]){
      setErrortext("لطفا همه ی فیلد ها را پر کنید")
    }else{
      setErrortext("")
      request("SettingsUpdateopticalroute",{data:{optical_route: {
  ...
  return (
    <SystemSettingsMain onResetButtonClick={onResetButtonClick} onSaveButtonClick={onSaveButtonClick}>
      ...
      <Rowinput name="Helix Factor">
        <TextInput value={helixfactor} onChange={(e)=>setHelixfactor(Number(e.target.value))} className="w-full" type="number" defaultValue={SettingsGet?.data?.optical_route?.helix_factor || 1.01} />
      </Rowinput>
        ...
        <Rowinput name={`IOR for 1310 nm wavelength`}>
          ...
          onChange={(e)=>{
            ...
            type="number"
            value={ior[1310]}
            defaultValue={SettingsGet?.data?.optical_route?.wavelengths?.IOR?.[1310]}
          />
        </Rowinput>
        <Rowinput name={`IOR for 1490 nm wavelength`}>
          <TextInput
            ...
            type="number"
            value={ior[1490]}
            defaultValue={SettingsGet?.data?.optical_route?.wavelengths?.IOR?.[1490]}
          />
        </Rowinput>
        ...
        <Rowinput name={`IOR for 1550 nm wavelength`}>
          ...
          onChange={(e)=>{
            ...
            type="number"
            value={ior[1550]}
            defaultValue={SettingsGet?.data?.optical_route?.wavelengths?.IOR?.[1550]}
          />
        </Rowinput>
        ...
        <Rowinput name={`IOR for 1625 nm wavelength`}>
          <TextInput
           onChange={(e)=>{
            ...
            type="number"
            value={ior[1625]}
            defaultValue={SettingsGet?.data?.optical_route?.wavelengths?.IOR?.[1625]}
          />
        </Rowinput>
        ...
        <Rowinput name={`RBS for 1310 nm wavelength`}>
          <TextInput
            ...
            type="number"
            value={rbs[1310]}
            defaultValue={SettingsGet?.data?.optical_route?.wavelengths?.RBS?.[1310]}
          />
        </Rowinput>
        ...
        <Rowinput name={`RBS for 1490 nm wavelength`}>
          <TextInput
           onChange={(e)=>{
            ...
            type="number"
            value={rbs[1490]}
            defaultValue={SettingsGet?.data?.optical_route?.wavelengths?.RBS?.[1490]}
          />
        </Rowinput>
        ...
        <Rowinput name={`RBS for 1550 nm wavelength`}>
          <TextInput
           onChange={(e)=>{
            ...
            type="number"
            value={rbs[1550]}
            defaultValue={SettingsGet?.data?.optical_route?.wavelengths?.RBS?.[1550]}
          />
        </Rowinput>
        <Rowinput name={`RBS for 1625 nm wavelength`}>
          <TextInput
            ...
            type="number"
            value={rbs[1625]}
            defaultValue={SettingsGet?.data?.optical_route?.wavelengths?.RBS?.[1625]}
          />
        </Rowinput>
   {errortext.length>0?
