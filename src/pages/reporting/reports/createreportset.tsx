...
import {alldeletereporttype, ReportsetReporttype} from '~/types/report';
import {
...
function CreateReportset() {
    ...
    if (name.length == 0) {
      ...
      try {
        ...
        if (cretereportset?.status == 201) {
          ...
          disptch(
            createreportsetlist({name: name, id: cretereportsetresponsdata}),
          );
  ...
  return (
    <div className="relative flex h-[100%-80px] w-full flex-col p-[20px]">
      <h1 className="mb-6 font-bold"> create reportset</h1>
      ...
      <div className="absolute bottom-0 right-0 flex flex-row">
        <SimpleBtn loading={loading} onClick={save}>
          Save
        </SimpleBtn>
